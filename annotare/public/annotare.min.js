...
return child};Flakey=require("flakey");$=Flakey.$;autoresize=require("../lib/autoresize");ui=require("../lib/uikit");settings=require("../settings");Document=require("../models/Document");Edit=function(){__extends(Edit,Flakey.controllers.Controller);function Edit(config){this.delete_note=__bind(this.delete_note,this);this.discard=__bind(this.discard,this);this.save=__bind(this.save,this);this.render=__bind(this.render,this);this.autosave=__bind(this.autosave,this);this.id="edit-view";this.class_name=
"edit_document view";this.actions={"click .save":"save","click .discard":"discard","click .delete":"delete_note","keyup #editor":"autosave"};Edit.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("edit",require("../views/edit"))}Edit.prototype.autosave=function(event){event.preventDefault();return localStorage[this.autosave_key()]=$("#editor").val()};Edit.prototype.autosave_key=function(){return"autosave-draft-"+this.id};Edit.prototype.render=function(){var context;var _this=
this;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)ui.confirm("Restore?","An unsaved draft of this note was found. Would you like to restore it to the editor?").show(function(ok){if(ok)return $("#editor").val(localStorage[_this.autosave_key()]);else return delete localStorage[_this.autosave_key()]});this.unbind_actions();this.bind_actions();
return $("#editor").autoResize({extraSpace:100,maxHeight:9E3})};Edit.prototype.save=function(event){event.preventDefault();this.doc.base_text=$("#editor").val();this.doc.save();delete localStorage[this.autosave_key()];ui.info("Everything's Shiny Capt'n!",'"'+this.doc.name+'" was successfully saved.').hide(settings.growl_hide_after).effect(settings.growl_effect);return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){var _this=
...
_this.doc.delete_annotation(id);return $(event.target).parent().slideUp()}})};return Edit}();module.exports=Edit}).call(this)});
