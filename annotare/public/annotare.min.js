...
return child};Flakey=require("flakey");$=Flakey.$;ui=require("../lib/uikit");settings=require("../settings");Document=require("../models/Document");Annotation=require("../models/Annotation");Detail=function(){__extends(Detail,Flakey.controllers.Controller);function Detail(config){this.drag_over=__bind(this.drag_over,this);this.drop_file=__bind(this.drop_file,this);this.delete_file=__bind(this.delete_file,this);this.annotate=__bind(this.annotate,this);this.highlight=__bind(this.highlight,this);this["delete"]=
__bind(this["delete"],this);this.edit_note=__bind(this.edit_note,this);this.edit=__bind(this.edit,this);this.render=__bind(this.render,this);this.id="detail-view";this.class_name="detail view";this.actions={"click .edit":"edit","click .highlighter":"highlight","click .annotate":"annotate","click .delete":"delete","click .delete-file":"delete_file","blur .note-detail":"edit_note"};Detail.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("detail",require("../views/detail"))}
Detail.prototype.render=function(){var context,dropZone;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);if(!(this.doc!=null))return;context={doc:this.doc};this.html(this.tmpl.render(context));this.unbind_actions();this.bind_actions();dropZone=document.getElementById("detail-drop-zone");dropZone.addEventListener("dragover",this.drag_over,false);return dropZone.addEventListener("drop",this.drop_file,false)};Detail.prototype.edit=function(event){event.preventDefault();return window.location.hash=
"#/edit?"+Flakey.util.querystring.build(this.query_params)};Detail.prototype.edit_note=function(event){var content,id,note,target;event.preventDefault();target=$(event.target);id=target.attr("id").replace("note-detail-","");content=target.html().replace(/\<br\/\>/,"\n\n").replace(/\<([\w\d\/]*)\>/g," ");note=Annotation.get(id);note.attachment=content;note.type="note";return note.save()};Detail.prototype["delete"]=function(event){var _this=this;event.preventDefault();return ui.confirm("Be careful!",
"Are you sure you want to delete this document?").show(function(ok){if(ok){_this.doc["delete"]();return window.location.hash="#/list"}})};Detail.prototype.highlight=function(event){var html,selection;event.preventDefault();selection=void 0;if(window.getSelection)selection=window.getSelection();else if(document.selection)selection=document.selection.createRange();selection=selection.toString();if(selection.length===0)return ui.error("Well, you were right about this being a bad idea.","Please select some text first.").hide(settings.growl_hide_after).effect(settings.growl_effect);
else{html=$("#detail-"+this.doc.slug).html();html=this.doc.annotate(selection,html);return $("#detail-"+this.doc.slug).html(html)}};Detail.prototype.annotate=function(event){var html,options,selection;event.preventDefault();selection=void 0;if(window.getSelection)selection=window.getSelection();else if(document.selection)selection=document.selection.createRange();selection=selection.toString();if(selection.length===0)return ui.error("Well, you were right about this being a bad idea.","Please select some text first.").hide(settings.growl_hide_after).effect(settings.growl_effect);
...
_this.doc.delete_file(id);return $(event.target).parent().slideUp()}})};Detail.prototype.drop_file=function(event){var file,files,_i,_len,_results;var _this=this;event.stopPropagation();event.preventDefault();files=event.dataTransfer.files;_results=[];for(_i=0,_len=files.length;_i<_len;_i++){file=files[_i];_results.push(this.doc.attach_file(file,function(file){$(".files ul").append("<li data-id='"+file.id+"'><a href='"+file.data+"' target='_blank'>"+file.name+" ("+file.mime+")</a>&nbsp;<a href='#' class='delete-file' data-id='"+
file.id+"'>[Delete]</a></li>");_this.unbind_actions();return _this.bind_actions()}))}return _results};Detail.prototype.drag_over=function(event){event.stopPropagation();event.preventDefault();return event.dataTransfer.dropEffect="copy"};return Detail}();module.exports=Detail}).call(this)});
...
__safe,__objSafe=__obj.safe,__escape=__obj.escape;__safe=__obj.safe=function(value){if(value&&value.ecoSafe)return value;else{if(!(typeof value!=="undefined"&&value!=null))value="";var result=new String(value);result.ecoSafe=true;return result}};if(!__escape)__escape=__obj.escape=function(value){return(""+value).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")};(function(){(function(){var file,_i,_len,_ref;__out.push('<div class="tool-bar-wrap">\n  <nav class="tool-bar left">\n    <div>\n      <a href="#" class="edit">Edit</a>\n      <a href="#/history?id=');
...
__bind(this.save,this);this.render=__bind(this.render,this);this.autosave=__bind(this.autosave,this);this.id="edit-view";this.class_name="edit_document view";this.actions={"click .save":"save","click .discard":"discard","click .delete":"delete_note","click .delete-file":"delete_file","keyup #edit-editor":"autosave"};Edit.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("edit",require("../views/edit"))}Edit.prototype.autosave=function(event){event.preventDefault();return localStorage[this.autosave_key()]=
$("#edit-editor").val()};Edit.prototype.autosave_key=function(){return"autosave-draft-"+this.doc.id};Edit.prototype.render=function(){var context,dropZone;var _this=this;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)ui.confirm("Restore?","An unsaved draft of this note was found. Would you like to restore it to the editor?").show(function(ok){if(ok)return $("#edit-editor").val(localStorage[_this.autosave_key()]);
else return delete localStorage[_this.autosave_key()]});this.unbind_actions();this.bind_actions();$("#edit-editor").autoResize({extraSpace:100,maxHeight:9E3});$("#edit-editor").blur().focus();dropZone=document.getElementById("edit-drop-zone");dropZone.addEventListener("dragover",this.drag_over,false);return dropZone.addEventListener("drop",this.drop_file,false)};Edit.prototype.save=function(event){event.preventDefault();this.doc.base_text=$("#edit-editor").val();this.doc.save();delete localStorage[this.autosave_key()];
ui.info("Everything's Shiny Capt'n!",'"'+this.doc.name+'" was successfully saved.').hide(settings.growl_hide_after).effect(settings.growl_effect);return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){if(ok){delete localStorage[_this.autosave_key()];
return window.location.hash="#/list"}})};Edit.prototype.delete_file=function(event){var _this=this;event.preventDefault();event.preventDefault();return ui.confirm("There be Monsters!","Are you sure you want to delete this annotation?").show(function(ok){var id;if(ok){id=$(event.target).attr("data-id");_this.doc.delete_file(id);return $(event.target).parent().slideUp()}})};Edit.prototype.delete_note=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Are you sure you want to delete this annotation?").show(function(ok){var id;
if(ok){id=$(event.target).parent().attr("data-id");_this.doc.delete_annotation(id);return $(event.target).parent().slideUp()}})};Edit.prototype.drop_file=function(event){var file,files,_i,_len,_results;var _this=this;event.stopPropagation();event.preventDefault();files=event.dataTransfer.files;_results=[];for(_i=0,_len=files.length;_i<_len;_i++){file=files[_i];_results.push(this.doc.attach_file(file,function(file){$(".files ul").append("<li data-id='"+file.id+"'><a href='"+file.data+"' target='_blank'>"+
file.name+" ("+file.mime+")</a>&nbsp;<a href='#' class='delete-file' data-id='"+file.id+"'>[Delete]</a></li>");_this.unbind_actions();return _this.bind_actions()}))}return _results};Edit.prototype.drag_over=function(event){event.stopPropagation();event.preventDefault();return event.dataTransfer.dropEffect="copy"};return Edit}();module.exports=Edit}).call(this)});
