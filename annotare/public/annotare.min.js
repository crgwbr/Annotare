...
this);this.autosave=__bind(this.autosave,this);this.id="edit-view";this.class_name="edit_document view";this.actions={"click .save":"save","click .discard":"discard","click .delete":"delete_note","keyup #edit-editor":"autosave"};Edit.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("edit",require("../views/edit"))}Edit.prototype.autosave=function(event){event.preventDefault();return localStorage[this.autosave_key()]=$("#edit-editor").val()};Edit.prototype.autosave_key=
function(){return"autosave-draft-"+this.doc.id};Edit.prototype.render=function(){var context,dropZone;var _this=this;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)ui.confirm("Restore?","An unsaved draft of this note was found. Would you like to restore it to the editor?").show(function(ok){if(ok)return $("#edit-editor").val(localStorage[_this.autosave_key()]);
else return delete localStorage[_this.autosave_key()]});this.unbind_actions();this.bind_actions();$("#edit-editor").autoResize({extraSpace:100,maxHeight:9E3});dropZone=document.getElementById("drop-zone");dropZone.addEventListener("dragover",this.drag_over,false);return dropZone.addEventListener("drop",this.drop_file,false)};Edit.prototype.save=function(event){event.preventDefault();this.doc.base_text=$("#edit-editor").val();this.doc.save();delete localStorage[this.autosave_key()];ui.info("Everything's Shiny Capt'n!",
'"'+this.doc.name+'" was successfully saved.').hide(settings.growl_hide_after).effect(settings.growl_effect);return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){if(ok){delete localStorage[_this.autosave_key()];return window.location.hash="#/list"}})};
Edit.prototype.delete_note=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Are you sure you want to delete this annotation?").show(function(ok){var id;if(ok){id=$(event.target).parent().attr("data-id");_this.doc.delete_annotation(id);return $(event.target).parent().slideUp()}})};Edit.prototype.drop_file=function(event){var file,files,output,_i,_len;event.stopPropagation();event.preventDefault();files=event.dataTransfer.files;output="";for(_i=0,_len=files.length;_i<
_len;_i++){file=files[_i];output+="<li>"+escape(file.name)+"</li>"}return $(".files ul").html(output)};Edit.prototype.drag_over=function(event){event.stopPropagation();event.preventDefault();return event.dataTransfer.dropEffect="copy"};return Edit}();module.exports=Edit}).call(this)});
