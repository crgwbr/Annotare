...
require.define("/models/Document.js",function(require,module,exports,__dirname,__filename){(function(){var Annotation,Document,Flakey,Showdown;var __bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};
Flakey=require("flakey");Showdown=require("../lib/showdown");Annotation=require("./Annotation");Document=function(){__extends(Document,Flakey.models.Model);function Document(){this.render=__bind(this.render,this);this.get_notes=__bind(this.get_notes,this);this.generate_slug=__bind(this.generate_slug,this);this.draw_annotations=__bind(this.draw_annotations,this);this.annotate=__bind(this.annotate,this);Document.__super__.constructor.apply(this,arguments)}Document.model_name="Document";Document.fields=
["id","name","slug","base_text","annotations"];Document.prototype.annotate=function(selection,html,attachment){var note,type;if(attachment===void 0)type="highlight";else type="note";note=new Annotation({text:selection,type:type,attachment:attachment,document:this.id});note.save();if(!this.annotations||this.annotations.constructor!==Array)this.annotations=[];this.annotations.push(note.id);this.save();return note.apply(html)};Document.prototype["delete"]=function(){var note,note_id,_i,_len,_ref;if(this.annotations!=
...
return child};Flakey=require("flakey");$=Flakey.$;ui=require("../lib/uikit");settings=require("../settings");Document=require("../models/Document");Annotation=require("../models/Annotation");Detail=function(){__extends(Detail,Flakey.controllers.Controller);function Detail(config){this.annotate=__bind(this.annotate,this);this.highlight=__bind(this.highlight,this);this["delete"]=__bind(this["delete"],this);this.edit_note=__bind(this.edit_note,this);this.edit=__bind(this.edit,this);this.render=__bind(this.render,
this);this.id="detail-view";this.class_name="detail view";this.actions={"click .edit":"edit","click .highlighter":"highlight","click .annotate":"annotate","click .delete":"delete","blur .note-detail":"edit_note"};Detail.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("detail",require("../views/detail"))}Detail.prototype.render=function(){var context;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);if(!(this.doc!=null))return;context={doc:this.doc};
this.html(this.tmpl.render(context));this.unbind_actions();return this.bind_actions()};Detail.prototype.edit=function(event){event.preventDefault();return window.location.hash="#/edit?"+Flakey.util.querystring.build(this.query_params)};Detail.prototype.edit_note=function(event){var content,id,note,target;event.preventDefault();target=$(event.target);id=target.attr("id").replace("note-detail-","");content=target.html().replace(/\<br\/\>/,"\n\n").replace(/\<([\w\d\/]*)\>/g," ");note=Annotation.get(id);
note.attachment=content;note.type="note";return note.save()};Detail.prototype["delete"]=function(event){var _this=this;event.preventDefault();return ui.confirm("Be careful!","Are you sure you want to delete this document?").show(function(ok){if(ok){_this.doc["delete"]();return window.location.hash="#/list"}})};Detail.prototype.highlight=function(event){var html,selection;event.preventDefault();selection=void 0;if(window.getSelection)selection=window.getSelection();else if(document.selection)selection=
...
window.getSelection();else if(document.selection)selection=document.selection.createRange();selection=selection.toString();if(selection.length===0)return ui.error("Well, you were right about this being a bad idea.","Please select some text first.").hide(settings.growl_hide_after).effect(settings.growl_effect);else{html=$("#detail-"+this.doc.slug).html();options={input:true,animate:true};html=this.doc.annotate(selection,html,"Click here to edit this note.");$("#detail-"+this.doc.slug).html(html);this.unbind_actions();
return this.bind_actions()}};return Detail}();module.exports=Detail}).call(this)});
...
return child};Flakey=require("flakey");$=Flakey.$;autoresize=require("../lib/autoresize");ui=require("../lib/uikit");settings=require("../settings");Document=require("../models/Document");Edit=function(){__extends(Edit,Flakey.controllers.Controller);function Edit(config){this.drag_over=__bind(this.drag_over,this);this.drop_file=__bind(this.drop_file,this);this.delete_note=__bind(this.delete_note,this);this.discard=__bind(this.discard,this);this.save=__bind(this.save,this);this.render=__bind(this.render,
this);this.autosave=__bind(this.autosave,this);this.id="edit-view";this.class_name="edit_document view";this.actions={"click .save":"save","click .discard":"discard","click .delete":"delete_note","keyup #editor":"autosave"};Edit.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("edit",require("../views/edit"))}Edit.prototype.autosave=function(event){event.preventDefault();return localStorage[this.autosave_key()]=$("#editor").val()};Edit.prototype.autosave_key=function(){return"autosave-draft-"+
this.doc.id};Edit.prototype.render=function(){var context,dropZone;var _this=this;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);context={doc:this.doc};this.html(this.tmpl.render(context));if(localStorage[this.autosave_key()]!=null&&localStorage[this.autosave_key()].length>0)ui.confirm("Restore?","An unsaved draft of this note was found. Would you like to restore it to the editor?").show(function(ok){if(ok)return $("#editor").val(localStorage[_this.autosave_key()]);else return delete localStorage[_this.autosave_key()]});
this.unbind_actions();this.bind_actions();$("#editor").autoResize({extraSpace:100,maxHeight:9E3});dropZone=document.getElementById("drop-zone");dropZone.addEventListener("dragover",this.drag_over,false);return dropZone.addEventListener("drop",this.drop_file,false)};Edit.prototype.save=function(event){event.preventDefault();this.doc.base_text=$("#edit-editor").val();this.doc.save();delete localStorage[this.autosave_key()];ui.info("Everything's Shiny Capt'n!",'"'+this.doc.name+'" was successfully saved.').hide(settings.growl_hide_after).effect(settings.growl_effect);
return window.location.hash="#/detail?"+Flakey.util.querystring.build(this.query_params)};Edit.prototype.discard=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard all changes to this document?").show(function(ok){if(ok){delete localStorage[_this.autosave_key()];return window.location.hash="#/list"}})};Edit.prototype.delete_note=function(event){var _this=this;event.preventDefault();return ui.confirm("There be Monsters!",
"Are you sure you want to delete this annotation?").show(function(ok){var id;if(ok){id=$(event.target).parent().attr("data-id");_this.doc.delete_annotation(id);return $(event.target).parent().slideUp()}})};Edit.prototype.drop_file=function(event){var file,files,output,_i,_len;event.stopPropagation();event.preventDefault();files=event.dataTransfer.files;output="";for(_i=0,_len=files.length;_i<_len;_i++){file=files[_i];output+="<li>"+escape(file.name)+"</li>"}return $(".files ul").html(output)};Edit.prototype.drag_over=
function(event){event.stopPropagation();event.preventDefault();return event.dataTransfer.dropEffect="copy"};return Edit}();module.exports=Edit}).call(this)});
...
require.define("/controllers/history.js",function(require,module,exports,__dirname,__filename){(function(){var $,Document,Flakey,History,Showdown,ui;var __bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;
return child};Flakey=require("flakey");$=Flakey.$;ui=require("../lib/uikit");Showdown=require("../lib/showdown");Document=require("../models/Document");History=function(){__extends(History,Flakey.controllers.Controller);function History(config){this.update=__bind(this.update,this);this.rollback=__bind(this.rollback,this);this.render=__bind(this.render,this);this.id="history-view";this.class_name="history view";this.actions={"change #version-input":"update","click #rollback":"rollback"};History.__super__.constructor.call(this,
