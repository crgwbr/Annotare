...
require.define("/controllers/new_document.js",function(require,module,exports,__dirname,__filename){(function(){var $,Document,Flakey,NewDocument,autoresize,settings,ui,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=
parent.prototype;return child};Flakey=require("flakey");$=Flakey.$;autoresize=require("../lib/autoresize");ui=require("../lib/uikit");settings=require("../settings");Document=require("../models/Document");NewDocument=function(_super){__extends(NewDocument,_super);function NewDocument(config){this.discard=__bind(this.discard,this);this.save=__bind(this.save,this);this.id="new-document-view";this.class_name="new_document view";this.actions={"click .new.save":"save","click .new.discard":"discard"};NewDocument.__super__.constructor.call(this,
config);this.tmpl=Flakey.templates.get_template("new_document",require("../views/new_document"))}NewDocument.prototype.render=function(){this.html(this.tmpl.render({title:this.query_params.title}));this.unbind_actions();this.bind_actions();$("#new-editor").autoResize({extraSpace:100,maxHeight:2E3});$("#new-editor").blur();return $("#name").focus()};NewDocument.prototype.save=function(params){var doc,name,text;name=$("#name").val();text=$("#new-editor").val();if(name.length>0&&text.length>0){doc=new Document({name:name,
base_text:text});doc.generate_slug();doc.save();ui.info("Everything's Shiny Capt'n!",'"'+doc.name+'" was successfully saved.').hide(settings.growl_hide_after).effect(settings.growl_effect);return window.location.hash="#/detail?"+Flakey.util.querystring.build({id:doc.id})}};NewDocument.prototype.discard=function(params){return ui.confirm("There be Monsters!","Careful there Captain; are you sure you want to discard this document?").show(function(ok){if(ok)return window.location.hash="#/list"})};return NewDocument}(Flakey.controllers.Controller);
module.exports=NewDocument}).call(this)});
...
for(_i=0,_len=_ref.length;_i<_len;_i++){id=_ref[_i];note=Annotation.get(id);if(note!=null)notes.push(note)}return notes};Document.prototype.render=function(){var converter,html;converter=new Showdown.converter;html=converter.makeHtml(this.base_text);return html};return Document}(Flakey.models.Model);module.exports=Document}).call(this)});
...
require.define("/models/File.js",function(require,module,exports,__dirname,__filename){(function(){var File,Flakey,__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};Flakey=require("flakey");File=function(_super){__extends(File,_super);function File(){File.__super__.constructor.apply(this,
arguments)}File.model_name="File";File.fields=["id","name","size","last_modified","mime","data","date_uploaded"];return File}(Flakey.models.Model);module.exports=File}).call(this)});
...
require("flakey");$=Flakey.$;Document=require("../models/Document");List=function(_super){__extends(List,_super);function List(config){this.search=__bind(this.search,this);this.new_document=__bind(this.new_document,this);this.render=__bind(this.render,this);this.id="list-view";this.class_name="list view";this.actions={"click .list.document":"select_doc","click .list.new_document":"new_document","keyup #search-box":"search"};List.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("list",
...
require.define("/controllers/detail.js",function(require,module,exports,__dirname,__filename){(function(){var $,Annotation,Detail,Document,Flakey,settings,ui,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;
return child};Flakey=require("flakey");$=Flakey.$;ui=require("../lib/uikit");settings=require("../settings");Document=require("../models/Document");Annotation=require("../models/Annotation");Detail=function(_super){__extends(Detail,_super);function Detail(config){this.delete_file=__bind(this.delete_file,this);this.annotate=__bind(this.annotate,this);this.highlight=__bind(this.highlight,this);this["delete"]=__bind(this["delete"],this);this.edit_note=__bind(this.edit_note,this);this.edit=__bind(this.edit,
this);this.render=__bind(this.render,this);this.id="detail-view";this.class_name="detail view";this.actions={"click .edit":"edit","click .highlighter":"highlight","click .annotate":"annotate","click .delete":"delete","click .delete-file":"delete_file","blur .note-detail":"edit_note","keyup ctrl+e":"edit","keyup ctrl+h":"highlight","keyup ctrl+n":"annotate","keyup ctrl+backspace":"delete"};Detail.__super__.constructor.call(this,config);this.tmpl=Flakey.templates.get_template("detail",require("../views/detail"))}
Detail.prototype.render=function(){var context;if(!this.query_params.id)return;this.doc=Document.get(this.query_params.id);if(!(this.doc!=null))return;context={doc:this.doc};this.html(this.tmpl.render(context));this.unbind_actions();return this.bind_actions()};Detail.prototype.edit=function(event){event.preventDefault();return window.location.hash="#/edit?"+Flakey.util.querystring.build(this.query_params)};Detail.prototype.edit_note=function(event){var content,id,note,target;event.preventDefault();
target=$(event.target);id=target.attr("id").replace("note-detail-","");content=target.html().replace(/\<br\/\>/,"\n\n").replace(/\<([\w\d\/]*)\>/g," ");note=Annotation.get(id);note.attachment=content;note.type="note";return note.save()};Detail.prototype["delete"]=function(event){var _this=this;event.preventDefault();return ui.confirm("Be careful!","Are you sure you want to delete this document?").show(function(ok){if(ok){_this.doc["delete"]();return window.location.hash="#/list"}})};Detail.prototype.highlight=
function(event){var html,selection;event.preventDefault();selection=void 0;if(window.getSelection)selection=window.getSelection();else if(document.selection)selection=document.selection.createRange();selection=selection.toString();if(selection.length===0)return ui.error("Well, you were right about this being a bad idea.","Please select some text first.").hide(settings.growl_hide_after).effect(settings.growl_effect);else{html=$("#detail-"+this.doc.slug).html();html=this.doc.annotate(selection,html);
return $("#detail-"+this.doc.slug).html(html)}};Detail.prototype.annotate=function(event){var html,options,selection;event.preventDefault();selection=void 0;if(window.getSelection)selection=window.getSelection();else if(document.selection)selection=document.selection.createRange();selection=selection.toString();if(selection.length===0)return ui.error("Well, you were right about this being a bad idea.","Please select some text first.").hide(settings.growl_hide_after).effect(settings.growl_effect);
...
_this.doc.delete_file(id);return $(event.target).parent().slideUp()}})};return Detail}(Flakey.controllers.Controller);module.exports=Detail}).call(this)});
...
_this.doc.delete_annotation(id);return $(event.target).parent().slideUp()}})};Edit.prototype.drop_file=function(event){var file,files,_i,_len,_results,_this=this;event.stopPropagation();event.preventDefault();files=event.dataTransfer.files;_results=[];for(_i=0,_len=files.length;_i<_len;_i++){file=files[_i];_results.push(this.doc.attach_file(file,function(file){$(".files ul").append("<li data-id='"+file.id+"'><a href='"+file.data+"' target='_blank'>"+file.name+" ("+file.mime+")</a>&nbsp;<a href='#' class='delete-file' data-id='"+
file.id+"'>[Delete]</a></li>");_this.unbind_actions();return _this.bind_actions()}))}return _results};Edit.prototype.drag_over=function(event){event.stopPropagation();event.preventDefault();return event.dataTransfer.dropEffect="copy"};return Edit}(Flakey.controllers.Controller);module.exports=Edit}).call(this)});
...
_ref2=this.doc.get_files();for(_j=0,_len2=_ref2.length;_j<_len2;_j++){file=_ref2[_j];__out.push('\n            <li data-id="');__out.push(__sanitize(file.id));__out.push('">\n              <a href="');__out.push(file.data);__out.push('" target="_blank">');__out.push(__sanitize(file.name));__out.push(" (");__out.push(__sanitize(file.mime));__out.push(')</a>\n              &nbsp;\n              <a href="#" class="edit delete-file" data-id="');__out.push(__sanitize(file.id));__out.push('">[Delete]</a>\n            </li>\n          ')}__out.push("\n        </ul>\n      </div>\n    </article>\n    \n  </section>\n</div>")}).call(this)}).call(__obj);
__obj.safe=__objSafe,__obj.escape=__escape;return __out.join("")}}).call(this)});
require.define("/controllers/history.js",function(require,module,exports,__dirname,__filename){(function(){var $,Document,File,Flakey,History,Showdown,ui,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}},__hasProp=Object.prototype.hasOwnProperty,__extends=function(child,parent){for(var key in parent)if(__hasProp.call(parent,key))child[key]=parent[key];function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;
return child};Flakey=require("flakey");$=Flakey.$;ui=require("../lib/uikit");Showdown=require("../lib/showdown");Document=require("../models/Document");File=require("../models/File");History=function(_super){__extends(History,_super);function History(config){this.update=__bind(this.update,this);this.rollback=__bind(this.rollback,this);this.render=__bind(this.render,this);this.id="history-view";this.class_name="history view";this.actions={"change #version-input":"update","click #rollback":"rollback"};
...
rev.files;_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++){file_id=_ref[_i];file=File.get(file_id);_results.push($("#history-files").append("<li data-id='"+file.id+"'><a target='_blank' href='"+file.data+"'>"+file.name+" ("+file.mime+")</a></li>"))}return _results};return History}(Flakey.controllers.Controller);module.exports=History}).call(this)});
